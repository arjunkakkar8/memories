...
function parseBatchScores(content: string): LlmCandidateScore[] {
	...
	return parsed.scores
		.map((entry) => {
			...
			const threadId =
				'threadId' in entry && typeof entry.threadId === 'string' ? entry.threadId : '';
			const score = 'score' in entry ? Number(entry.score) : Number.NaN;
			const rationale =
				'rationale' in entry && typeof entry.rationale === 'string' ? entry.rationale : '';
			const titleRaw =
			...
			return {
				...
				themes,
				title: titleRaw.length > 0 ? titleRaw.slice(0, 120) : null
			};
		})
...
export async function scoreCandidateBatch(
	...
	const promptPayload = candidates.map((candidate) => ({
		...
		recipients: candidate.metadata.participants,
		senderAddresses: candidate.metadata.senderAddresses,
		senderDomains: candidate.metadata.senderDomains,
		messageCount: candidate.metadata.messageCount,
		firstMessageAt: candidate.metadata.firstMessageAt,
		lastMessageAt: candidate.metadata.lastMessageAt,
				...
				messages: [
					{
						role: 'system',
						content:
							'You rank Gmail thread metadata for narrative potential and create concise card titles. Reply only JSON with {"scores":[{"threadId":"...","score":0..1,"rationale":"...","themes":["..."],"title":"..."}]}. Preserve every threadId exactly once. Titles should be specific, 4-10 words, and reflect people plus context without RE or FWD prefixes.'
					},
					{
						role: 'user',
